...
function getRawHtmlReportUrl(report: WorkReport): string | null {
  if (report.report_type === "html" && report.slug) {
    return `/reports/raw/${report.slug}`
  }
  return null
}
...
export default function OpsProjectPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100">
        <div className="max-w-3xl mx-auto px-4 py-10 space-y-4">
          <Link href="/ops" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-200">
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 md:p-6 space-y-4">
            ...
            {reports.length === 0 ? (
              ...
              <div className="space-y-2">
                {reports.map((report) => {
                  const reportHref = getPreferredReportUrl(report)
                  const rawHtmlHref = getRawHtmlReportUrl(report)
                  ...
                  return (
                    <article key={report.id} className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                        {isInternalUrl(reportHref) ? (
                          <Link href={reportHref} className="inline-flex items-center gap-1 text-sky-300 hover:text-sky-200">
                            Open report
                          </Link>
                        ) : (
                          <a
